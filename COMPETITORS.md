...
**[mentlio.com](https://www.mentlio.com/)** is the closest match to RouteWise's thesis found so far — close enough that it should be treated as the primary competitor to design against, not an honorable mention. Full archived docs and benchmark write-ups (pulled from their site, July 2026) live in [`research/mentlio/`](./research/mentlio/):
...
- **Overlap with RouteWise**: Directly matches Phase 2 ("Routing layer") — picking the cheapest model that reliably clears the bar for a given task, automatically and per-call.
- **Gap vs. RouteWise's thesis**: Operates at the *query* level, not the *workflow-decomposition* level. It optimizes "which model should answer this prompt," not "should this step even be a model call, or code/cache instead." No explicit deterministic-code or caching layer — it's a router, not a decomposition engine.
...
- **Overlap with RouteWise**: Matches Phases 2–4 closely — routing (cheapest sufficient model), caching (Phase 3, "reuse known outputs instead of regenerating"), and some context/observability tooling (Phase 4/5). Widely cited as the top pick for "enterprise compliance" among LLM routers in 2026.
- **Gap vs. RouteWise's thesis**: A generic gateway/infra layer, not opinionated about *task classification* (deterministic vs. cacheable vs. simple-judgment vs. complex-judgment) or about producing an audit/savings report per workflow. It's plumbing a team wires up themselves; RouteWise's wedge (per the plan's "Product wedge A") is the analysis and classification on top of that plumbing.
...
- **Overlap with RouteWise**: Closest in *audience and framing* to Varick Agents' own pitch — enterprise, agent-specific, spend-control-first, marketed as "prevent runaway agent spend." Matches Phase 5 ("Measurement / ROI dashboard") most directly.
- **Gap vs. RouteWise's thesis**: Fundamentally a *governance/circuit-breaker* layer (limit and stop excess spend after the fact) rather than a *routing/decomposition* layer (avoid the spend in the first place by never calling a frontier model when code or a cheap model would do). Reactive cost control, not proactive task-to-resource matching.
